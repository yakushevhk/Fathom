...
All built-in tools are available (63 always-registered built-ins, plus optional browser/computer tools and LSP): `file_read`, `file_write`, `file_edit`, `edit`, `glob`, `grep`, `shell`, `python_exec`, `web_search`, `spawn_agent`, `hub`, `daemon`, `compiler_check`, `repro_test`, `debug`, `ast_edit`, and many more. The exact list depends on the build configuration and environment — inspect the `tools/list` response from `fathom mcp-serve` for the authoritative set.
