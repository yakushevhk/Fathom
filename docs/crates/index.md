...
| **`pr-lsp`** | *(in crate source)* | **Language Server Protocol** integration: `client.rs` — LSP client connection management, `detect.rs` — automatic detection of LSP-capable language servers, `install.rs` — server installation and configuration, `tool.rs` — LSP tool integration for code intelligence (go-to-definition, references, hover, completion). Enables IDE-like features within the agent's tool ecosystem. |
...
| `mcp-serve` | MCP server — exposes all agent tools to external MCP clients (Claude, ZCode, etc.) over stdio. Executes tools/call via the shared ToolRegistry | mcp, tools |
...
|---|---|---|
| **10 crates** | Clear separation of responsibilities into 10 workspace crates | Enforces dependency direction, enables independent compilation and testing, allows crate-level feature flags (e.g., `postgres` feature in `pr-persistence`) |
