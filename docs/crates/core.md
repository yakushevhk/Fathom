...
3. Embed into a full HTML template with CSS styles:
   ...
   - Meta viewport tags
4. Add footer: `"Generated by Parallel Research Agent · session {id} · {timestamp}"`
...
2. If `email_to` is not empty → add `Email` channel. `smtp_host` defaults to `"localhost"`, `email_from` defaults to `"parallel-research@localhost"`
