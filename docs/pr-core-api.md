...
> The `pr-core` crate is the fundamental core of the Parallel Research Agent system.
...
This means consumers of the crate can write `use pr_core::AgentId` directly, without specifying the full path `pr_core::ids::AgentId`.
...
5. If `build()` fails, returns the default `reqwest::Client::new()` via `unwrap_or_else`

**Edge cases**: If `build()` fails, the default client has no timeouts — this is an emergency fallback.

**Interaction**: Used by `CrmSync`, `Notifier`, and by search and LLM provider crates. Intended to replace `reqwest::Client::new()` throughout the codebase.

---

## 2. ids.rs

This module contains three type-safe newtype wrappers for identifiers. All three follow the same structure.
...
Generates a UUIDv7 via `Uuid::now_v7()`, converts to string. UUIDv7 contains a timestamp with millisecond precision in the high bits, providing sortability (monotonic ordering).
...
**Interaction**: All three types implement `Hash`, `Eq`, `Clone` and `Serialize`/`Deserialize`, allowing their use in HashMap, databases, and serialization.
...
Arbitrary content + list of tool calls. `content` can be `None`.
...
**Interaction**: Used by the LLM module when building dialog history and by the agent module for passing results.
...
**Algorithm**:
1. Converts `id` and `name` into `String`
2. Sets `call_type = "function"`
3. Serializes `arguments` into a JSON string via `serde_json::to_string`; on error — empty string
...
| `Verifier` | Verifies finding credibility | ❌ |
...
Agent's current state. Uses discriminant serialization with the `"state"` field.
...
Agent lifecycle (as opposed to `AgentState`, which describes the current work phase).
...
Full agent record, persisted in the database.
...
| `depth` | `u32` | Depth in tree (0 = root) |
| `tokens_used` | `u64` | Tokens consumed |
| `created_at` | `DateTime<Utc>` | Creation time |
| `completed_at` | `Option<DateTime<Utc>>` | Completion time |

**Interaction**: Persisted in SQLite/PostgreSQL via `pr-persistence`.
...
Union type of all system events. Each variant has a unique `"type"` tag value.
...
**Interaction**: Published via `AgentEvent::Finding`, persisted in `findings/*.md`.
...
Tool schema in OpenAI `tools` API format. `parameters` is an arbitrary JSON Schema.
...
Root configuration. Loaded from `~/.parallel-research/config.toml`.
...
Returns `~/.parallel-research/config.toml`. Error if no home directory.

#### `AppConfig::save(&self) -> anyhow::Result<()>`

Serializes to TOML, writes to file. Creates the directory if needed.
...
- `deny_tools` — tools denied per role (e.g., `verifier = ["shell"]`)
- `role_models` — model overrides per role (e.g., `researcher = "cheap-model"`)
- `session_token_limit` — token limit per session (0 = disabled)
...
Sets a configuration value by dotted path (e.g., `llm.api_key`).

**Algorithm (step-by-step)**:
1. **Path**: `AppConfig::config_path()` → `~/.parallel-research/config.toml`
2. **Load**: If file exists — parse into `toml::Value`; otherwise — empty table
...
4. **Value parsing**: Order: `bool` → `i64` → `f64` → `String`. If the existing value is a string — force as string
5. **Insertion**: `set_nested_value(&mut root, key, parsed)` — creates intermediate tables if needed
6. **Validation**: Serializes back to TOML, attempts to deserialize into `AppConfig`. Error → file is not written
...
Descends by dotted path into the TOML tree. Returns `None` if segment not found.
...
3. ASCII: `(count + 3) / 4` (4 characters ≈ 1 token, rounding up)
...
For each schema: serializes to JSON → estimates tokens. Sums them up.
...
File-based memory storage. Entries are separated by `§` in files. Character budget limits injection into the prompt.

#### `MemoryStore::new(home_dir) -> Self`

Path: `home_dir/.parallel-research/memory/{MEMORY,USER}.md`. Budgets: 2200 / 1375.
...
Analogous for `USER.md`.

#### `MemoryStore::load_entries_from_path(path) -> Vec<MemoryEntry>` (static, public)

Loads from an arbitrary path.

#### `MemoryStore::add_memory(content) -> Result<()>`

**Algorithm** (`add_to_file`):
1. Trim; empty → error
2. Loads existing entries
3. Exact duplicate → `Ok(())` (idempotency)
4. Adds new entry
5. Serializes via `§\n`
6. Applies budget (`enforce_budget`) — removes old ones if exceeded
...
**Algorithm**:
1. Loads entries
2. Finds the first one containing `old_substr`
3. Not found → error
4. Replaces, applies budget, atomically writes
...
**Algorithm**:
1. Loads both files into memory
2. Applies each operation:
   - **Add**: checks for duplicate, adds
   - **Replace**: searches by `old_text`, replaces
   - **Remove**: removes by `old_text`; not found → entire batch fails
3. If error → **entire batch is aborted**, files are not written
4. Serializes both files, applies budgets
5. Atomically writes both files

**Atomicity guarantee**: If an error occurs mid-way, changes are not written. Both files are written sequentially (not truly atomic across both).

#### `MemoryStore::to_system_prompt_block() -> String`

Builds a block for the system prompt:
```
## Memory
...
**Algorithm**:
1. If `len <= max_chars` → returns as-is
2. Splits by `§`
3. Iterates **from the end** (newest): collects entries until budget is exhausted
4. Reverses the result (since it went from the end)
...
2. Writes to a temporary file `*.md.tmp`
3. `rename` temporary → target (atomic FS operation)
...
3. If name is empty → fallback to directory name → `"unnamed"`
4. `content` = full file contents

### `parse_skill_header(content) -> (String, String)` (private)

Looks for `# ` header, then the first non-empty non-heading line as the description.
...
`skills_dir = home_dir/.parallel-research/skills/`.
...
**Algorithm**:
1. Clears the skill list
2. If directory does not exist → Ok
3. Recursively scans the directory
4. For each `SKILL.md` file → `Skill::from_file(&path)`
5. Load error → warning in log, continues
...
**Algorithm**:
1. Generates a slug from `task` via `slugify()`
2. Creates directory `skills_dir/<slug>/`
3. Writes `SKILL.md` with format:
   ```
   # <task>
   ...
   <approach>
   ```
4. Adds the skill to the registry
...
2. Splits by `-`, filters empties, joins with `-`
...
2. If text is longer → appends `…` (Unicode ellipsis)

**Edge cases**: Empty synthesis → empty string. `max_chars = 0` → `"…"`.

**Interaction**: Used by notifications to build email/Telegram/webhook text.
...
Returns an error describing valid values.
...
2. Delegates to `export_html`, `export_json`, `export_pdf`, `export_docx`
3. Returns the path to the created file
...
**Algorithm**: Creates the directory, delegates to the format-specific method.

### HTML Export (`export_html`)

**Algorithm**:
1. Calls `build_report_markdown(session)` to generate Markdown
2. Calls `render_html_document(session, &markdown)` to render into HTML
3. Writes to `output_dir/report.html`

### `render_html_document(session, markdown) -> String`

**Algorithm**:
1. Creates a `pulldown_cmark::Parser` with extensions: tables, footnotes, strikethrough, tasklists
2. Renders Markdown to HTML via `html::push_html`
3. Embeds into a full HTML template with CSS styles:
   - System fonts (Apple, Segoe UI, Roboto)
   - Max width 860px, centered
   - Styles for `h1`, `h2`, `h3`, `code`, `pre`, `blockquote`, `table`, `a`, `hr`
   - CSS `color-scheme: light dark` for dark mode support
   - Viewport meta tags
4. Adds footer: `"Generated by Parallel Research Agent · session {id} · {timestamp}"`
5. Escapes `session_id` via `html_escape` (replaces `&`, `<`, `>`, `"`)
...
**Algorithm**:
1. Collects findings via `collect_findings`
...
3. Builds JSON:
   ...
   ```
4. Writes to `report.json`

### PDF Export (`export_pdf`)

**Algorithm**:
1. Writes Markdown to a temporary `report.md`
2. **Priority 1**: If `pandoc` is available → calls `pandoc report.md -o report.pdf --standalone`
3. If pandoc failed:
   - **Priority 2**: If `wkhtmltopdf` is available → exports HTML, then `wkhtmltopdf report.html report.pdf`
...
**Algorithm**:
1. Checks for pandoc; if missing → error
2. Writes Markdown
3. Calls `pandoc report.md -o report.docx`

### `build_report_markdown(session) -> String`

**Algorithm**:
1. Header: `# Research Report`
2. Metadata: Session, Generated, Agents, Tokens
3. Horizontal rule
4. Body: `session.synthesis`
5. Appendix: collects findings via `collect_findings`, builds `## Appendix: Individual Findings` with `### filename` for each

### `collect_findings(output_dir) -> Vec<(String, String)>`

**Algorithm**:
1. Reads the directory `output_dir/findings/`
2. Filters files with `.md` extension
3. Sorts by name
4. Returns `(filename, content)` for each
...
**Algorithm**:
1. Searches for occurrences of `https://` and `http://`
2. For each: finds the end of the URL (space, `<`, `>`, `"`, `'`, `)`, `]`, `` ` ``)
3. Strips trailing `.` `,` `;` `:`
4. Deduplicates (does not add an already existing URL)
...
**RFC 4180 quoting**:
- If it contains `,`, `"`, `\n`, `\r` → wraps in `"..."`, all `"` inside are replaced with `""`
...
**Algorithm**:
1. Creates `rust_xlsxwriter::Workbook`
2. Adds worksheet `"Contacts"`
3. Writes headers (row 0)
4. For each contact (row N+1):
   - `id` → `write_number` (as f64)
   - Other fields → `write_string`
   - `social_profiles` → `platform:url/username`, via `"; "`
   - `tags` → via `"; "`
5. `workbook.save_to_buffer()` → returns XLSX bytes
...
Creates with the given list of channels. HTTP client via `http_client()`.

#### `Notifier::from_config(config) -> Self`

**Algorithm**:
1. If `webhook_url` is not empty → adds `Webhook` channel
2. If `email_to` is not empty → adds `Email` channel. `smtp_host` defaults to `"localhost"`, `email_from` defaults to `"parallel-research@localhost"`
3. If both `telegram_bot_token` and `telegram_chat_id` are not empty → adds `Telegram` channel
...
3. Errors are **collected**, do not interrupt other channels
4. If there are errors → returns `anyhow::Error` with a list of failures

**Guarantee**: An error on one channel does not prevent delivery through others.
...
1. Builds JSON payload:
   ...
   ```
2. POST to URL
3. Checks HTTP status → error if not success

### `send_email(smtp_host, smtp_port, from, to, username, password, session)` (private)

**Algorithm**:
1. Builds email via `lettre::Message::builder`:
...
2. JSON: `{"chat_id": "...", "text": "..."}`
3. Checks HTTP status and `"ok": true` field in the response
4. Error → description from Telegram's `"description"` field
...
Contact collected during research.

| Field | Description |
|-------|-------------|
| `id` | DB ID (`None` until saved) |
...
**Interaction**: Used for deduplication: two contacts with the same set of phone digits are considered duplicates.
...
Creates from `CrmConfig`. `None` if provider is not configured.

#### `CrmSync::with_endpoint(endpoint) -> Self`

URL override (for testing with mock server).
...
4. POST request
5. Extracts ID from response: `/_embedded/contacts/0/id` or `id`
...
**Algorithm**:
1. Builds an array with one object
...
4. Checks HTTP status and presence of `"error"` in response
5. Extracts ID from `"result"`
...
First word → first name. Remaining words → last name (optional).

#### `id_to_string(v) -> String` (private)

Converts `serde_json::Value` (Number/String) to a string.
...
**Deduplication via `crm_id`**: The `Contact::crm_id` field stores the CRM contact ID after the first push. Before a subsequent push, the system checks whether `crm_id` is filled — if so, the contact is updated instead of creating a new one. This prevents duplicate contacts in the CRM during repeated research sessions.
...
- API errors with verification of informative error message generation
- Correct URL and payload construction for each provider
...
The following describes how modules within `pr-core` interact with each other and with external crates.
