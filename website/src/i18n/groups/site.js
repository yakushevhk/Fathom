...
export default {
  install: {
    ...
    env: {
      ...
      9: { purpose: { en: 'Config file path override (default: ~/.fathom-research/config.toml).', ru: 'Переопределение пути к конфигурационному файлу (по умолчанию: ~/.fathom-research/config.toml).' } },
  ...
  pricing: {
    hero: {
      ...
      sub: { en: 'every deployment ships the full binary — 50+ tools, 10 crates, no usage metering', ru: 'каждый деплой поставляет полный бинарник — 50+ инструментов, 10 крейтов, без учёта использования' },
    },
    ...
    tier: {
      ...
      0: {
        ...
        feat: {
          ...
          1: { en: '<kbd>Tools</kbd> All 50+, no metering', ru: '<kbd>Инструменты</kbd> Все 50+, без учёта' },
    ...
    compare: {
      ...
      sub: {
        en: 'Every tier ships all 50+ tools — plans differ in seats, storage, integration depth and support, never in capability.',
        ru: 'Каждый тариф поставляет все 50+ инструментов — планы различаются местами, хранилищем, глубиной интеграции и поддержкой, но не функциональностью.',
      },
      tab: {
    ...
    faq: {
      ...
      0: {
        ...
        a: {
          en: 'Fathom Research is a closed product distributed individually — no public package registries or app stores. You receive a single static binary containing the CLI, TUI, HTTP server, dashboard, MCP server and worker processes. Every deployment is complete and identical: 50+ tools, 10 crates, no usage metering.',
          ru: 'Fathom Research — закрытый продукт, распространяемый индивидуально — без публичных реестров пакетов и app stores. Вы получаете единый статический бинарник, содержащий CLI, TUI, HTTP-сервер, дашборд, MCP-сервер и воркер-процессы. Каждый деплой полный и идентичный: 50+ инструментов, 10 крейтов, без учёта использования.',
        },
      },
      1: {
  ...
  changelog: {
    ...
    note: {
      en: '* Full history ships with your deployment — run <code>fathom-research</code> on your own infrastructure; every build carries its own log.',
      ru: '* Полная история поставляется с вашим деплоем — запускайте <code>fathom-research</code> на своей инфраструктуре; каждая сборка несёт свой собственный лог.',
    },
    cta: {
  ...
  postBench: {
    ...
    p: {
      ...
      11: {
        en: 'Everything above reproduces with one command — <code>fathom-research bench --scenario all</code> — with fixtures that create and clean themselves. The scenarios: dispatch, parallel-io, parallel-cpu, mixed, parse-scale, extract-json, feed-parse, code-map and memory. Because nothing touches the network and nothing calls an LLM, the suite runs in CI on every build — which is exactly how the next silent regression gets caught.',
        ru: 'Всё вышеперечисленное воспроизводится одной командой — <code>fathom-research bench --scenario all</code> — с фикстурами, которые создаются и очищаются сами. Сценарии: dispatch, parallel-io, parallel-cpu, mixed, parse-scale, extract-json, feed-parse, code-map и memory. Поскольку ничто не касается сети и ничто не вызывает LLM, набор выполняется в CI на каждой сборке — именно так ловится следующая тихая регрессия.',
      },
      12: {
        en: 'And because synthetic numbers are only half the story, <code>fathom-research stats</code> reads the SQLite tracing of a real session and reports per-tool p50/p95 durations and the batching coefficient from production runs.',
        ru: 'А поскольку синтетические цифры — лишь половина истории, <code>fathom-research stats</code> читает SQLite-трассировку реальной сессии и сообщает длительности p50/p95 по каждому инструменту и коэффициент батчинга из продакшн-запусков.',
      },
    },
    t2: {
