...
impl Sidebar {
    ...
    fn render_agent_profile_dialog(&self, agent_id: &str, cx: &mut Context<Self>) -> Div {
        let (name, title, prompt) = match agent_id {
            "risk_analyst" => (
                "Risk Analyst",
                "🛡️ Compliance & Audit Officer",
                "Evaluate actions against CEL boundaries and ensure zero secret leaks.",
            ),
            "devops_engineer" => (
                "DevOps Engineer",
                "🐳 Infrastructure & Automation",
                "Manage containers, deployments, health probes and process trees.",
            ),
            _ => (
                "General Assistant",
                "🤖 General Autonomous Worker",
                "Full computer use, research, web automation, data synthesis and lead finding.",
            ),
        };
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .child(if coworkers.is_empty() {
                        div()
                            .flex()
                            .flex_col()
                            .gap_1()
                            .child(self.render_clickable_coworker(
                                "general_assistant",
                                "General Assistant",
                                "SDR & Web Research",
                                "🟢 Online",
                        ...
                        for cw in coworkers {
                            cow_elements = cow_elements.child(self.render_clickable_coworker(
                                &cw.id,
                                &cw.name,
                                &cw.title,
                                "🟢 Online",
            ...
            .children(self.selected_agent_for_dialog.as_ref().map(|agent_id| {
                div()
                    .p_3()
                    .child(self.render_agent_profile_dialog(agent_id, cx))
            }))
