...
impl From<pr_persistence::CredentialRow> for CredentialResponse {
    fn from(row: pr_persistence::CredentialRow) -> Self {
        Self {
            id: row.id,
            name: row.name,
            kind: row.kind,
...
pub async fn store(
    ...
    match state
        .db
        .store_credential(&body.name, &body.kind, &body.secret)
    {
        Ok(row) => (StatusCode::CREATED, Json(CredentialResponse::from(row))).into_response(),
...
pub async fn delete(State(state): State<Arc<AppState>>, Path(id): Path<String>) -> Response {
    match state.db.delete_credential(&id) {
        ...
        Ok(false) => error(StatusCode::NOT_FOUND, "credential not found"),
        Err(e) => error(StatusCode::BAD_REQUEST, e.to_string()),
    }
}
