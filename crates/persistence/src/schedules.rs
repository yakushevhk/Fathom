...
fn required(value: &str, max: usize, field: &str) -> Result<String> {
    ...
    if value.chars().count() > max {
        bail!("{field} exceeds maximum length of {max}");
    }
    Ok(value.to_owned())
}
...
pub fn validate_cron(value: &str) -> Result<String> {
    ...
    if value.starts_with('@') {
        bail!("cron_expression must contain five fields");
    }
...
fn parse_field(value: &str, min: u32, max: u32) -> Result<Vec<u32>> {
    ...
    for item in value.split(',') {
        let (base, step) = match item.split_once('/') {
            Some((base, step)) => (
                base,
                step.parse::<u32>().map_err(|_| anyhow!("invalid step"))?,
            ),
            None => (item, 1),
        };
        if step == 0 {
        ...
        let (lo, hi) = if base == "*" {
            ...
            (
                a.parse().map_err(|_| anyhow!("invalid range"))?,
                b.parse().map_err(|_| anyhow!("invalid range"))?,
            )
        } else {
            let n = base.parse::<u32>().map_err(|_| anyhow!("invalid value"))?;
            (n, n)
        };
        if lo < min || hi > max || lo > hi {
            bail!("value outside allowed range");
        }
        let mut n = lo;
        while n <= hi {
            out.push(n);
...
pub fn validate_timezone(value: &str) -> Result<String> {
    ...
    if value == "UTC" || value == "Etc/UTC" || parse_offset(&value).is_some() {
        return Ok(value);
    }
    if value.starts_with('/') || value.contains("..") || !value.contains('/') {
        bail!("timezone must be UTC, a fixed offset, or an IANA timezone");
    }
    if !Path::new("/usr/share/zoneinfo").join(&value).is_file() {
        bail!("unknown timezone");
    }
    Ok(value)
}
fn parse_offset(tz: &str) -> Option<i32> {
    ...
    if let Some(rest) = tz
        ...
        .or_else(|| tz.strip_prefix("Etc/GMT-"))
    {
...
fn next_occurrence(cron: &str, timezone: &str, after: DateTime<Utc>) -> Result<String> {
    ...
    let fields = [(0, 59), (0, 23), (1, 31), (1, 12), (0, 7)]
        .iter()
        .zip(raw)
        .map(|(&(lo, hi), v)| parse_field(v, lo, hi))
        .collect::<Result<Vec<_>>>()?;
    let offset = parse_offset(timezone).unwrap_or(0);
    let local_start = after
        .naive_utc()
        .with_second(0)
...
fn from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<ScheduleRow> {
    Ok(ScheduleRow {
        id: row.get(0)?,
...
impl Persistence {
    pub fn create_schedule(
        ...
        ensure_schema(self)?;
        let coworker_id = valid_id(coworker_id)?;
        let cron_expression = validate_cron(cron_expression)?;
        let timezone = validate_timezone(timezone)?;
        let query = required(query, MAX_QUERY, "query")?;
        let now = Utc::now();
        let next_run = match next_run {
            Some(v) => DateTime::parse_from_rfc3339(v)?
                .with_timezone(&Utc)
                .to_rfc3339(),
            None => next_occurrence(&cron_expression, &timezone, now)?,
        };
        ...
        if !exists {
            bail!("coworker not found");
        }
        conn.execute("INSERT INTO schedules (id,coworker_id,cron_expression,timezone,query,enabled,next_run,created_at,updated_at) VALUES (?1,?2,?3,?4,?5,?6,?7,?8,?8)",params![id,coworker_id,cron_expression,timezone,query,enabled as i64,next_run,now_s])?;
        Ok(conn.query_row(&format!("{SELECT} WHERE id=?1"), params![id], from_row)?)
    }
    pub fn list_schedules(&self) -> Result<Vec<ScheduleRow>> {
        ensure_schema(self)?;
        let conn = self.conn.lock();
        let mut stmt = conn.prepare(&format!("{SELECT} ORDER BY next_run,id"))?;
        let rows = stmt
            .query_map([], from_row)?
    ...
    pub fn update_schedule(
        ...
        let changed=conn.execute("UPDATE schedules SET coworker_id=?1,cron_expression=?2,timezone=?3,query=?4,enabled=?5,next_run=?6,updated_at=?7 WHERE id=?8",params![coworker_id,cron_expression,timezone,query,enabled as i64,next_run,now_s,id])?;
        if changed == 0 {
            return Ok(None);
        }
    ...
    pub fn claim_due_schedules(
        ...
        ensure_schema(self)?;
        let limit = limit.min(MAX_CLAIM);
        let now_s = now.to_rfc3339();
        let mut conn = self.conn.lock();
        let tx = conn.transaction_with_behavior(rusqlite::TransactionBehavior::Immediate)?;
        let mut stmt = tx.prepare(&format!(
            "{SELECT} WHERE enabled=1 AND next_run<=?1 ORDER BY next_run,id LIMIT ?2"
        ))?;
        let rows = stmt
            .query_map(params![now_s, limit as i64], from_row)?
            .collect::<rusqlite::Result<Vec<_>>>()?;
        drop(stmt);
        let mut claimed = Vec::with_capacity(rows.len());
        for row in rows {
            let next = match next_occurrence(&row.cron_expression, &row.timezone, now) {
                Ok(n) => n,
                Err(e) => {
                    tracing::error!(schedule_id=%row.id, error=%e, "failed to compute next occurrence for schedule; disabling");
                    let _ = tx.execute(
            ...
            let changed=tx.execute("UPDATE schedules SET next_run=?1,last_run=?2,updated_at=?2 WHERE id=?3 AND next_run=?4 AND enabled=1",params![next,now_s,row.id,row.next_run])?;
            if changed > 0 {
                claimed.push(ScheduleRow {
                    next_run: next,
                    last_run: Some(now_s.clone()),
...
mod tests {
    use super::*;
    #[test]
    fn schedule_crud_and_atomic_claim() {
        let db = Persistence::in_memory().unwrap();
        let c = db
            .create_coworker("Alice", "", "", "prompt", "private", true)
            .unwrap();
        let due = (Utc::now() - Duration::minutes(2)).to_rfc3339();
        let row = db
            .create_schedule(&c.id, "* * * * *", "UTC", "do work", true, Some(&due))
            .unwrap();
        assert_eq!(db.list_schedules().unwrap().len(), 1);
        let got = db.claim_due_schedules(Utc::now(), 10).unwrap();
        assert_eq!(got.len(), 1);
        assert_eq!(got[0].id, row.id);
        assert!(db.claim_due_schedules(Utc::now(), 10).unwrap().is_empty());
        assert!(
            !db.update_schedule(&row.id, &c.id, "*/5 * * * *", "UTC", "updated", false, None)
                .unwrap()
                .unwrap()
                .enabled
        );
        assert!(db.delete_schedule(&row.id).unwrap());
    }
    #[test]
    fn validation() {
        assert!(validate_cron("* * * * *").is_ok());
        assert!(validate_cron("/bin/sh -c x").is_err());
