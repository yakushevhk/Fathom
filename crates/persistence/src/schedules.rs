...
impl Persistence {
    ...
    pub fn claim_due_schedules(&self,now:DateTime<Utc>,limit:usize)->Result<Vec<ScheduleRow>> { if limit == 0 { return Ok(Vec::new()); } ensure_schema(self)?; let limit=limit.min(MAX_CLAIM); let now_s=now.to_rfc3339(); let mut conn=self.conn.lock(); let tx=conn.transaction_with_behavior(rusqlite::TransactionBehavior::Immediate)?; let mut stmt=tx.prepare(&format!("{SELECT} WHERE enabled=1 AND next_run<=?1 ORDER BY next_run,id LIMIT ?2"))?; let rows=stmt.query_map(params![now_s,limit as i64],from_row)?.collect::<rusqlite::Result<Vec<_>>>()?; drop(stmt); let mut claimed=Vec::with_capacity(rows.len()); for row in rows { let next = match next_occurrence(&row.cron_expression,&row.timezone,now) { Ok(n) => n, Err(e) => { tracing::error!(schedule_id=%row.id, error=%e, "failed to compute next occurrence for schedule; disabling"); let _ = tx.execute("UPDATE schedules SET enabled=0,updated_at=?1 WHERE id=?2", params![now_s, row.id]); continue; } }; let changed=tx.execute("UPDATE schedules SET next_run=?1,last_run=?2,updated_at=?2 WHERE id=?3 AND next_run=?4 AND enabled=1",params![next,now_s,row.id,row.next_run])?; if changed>0 { claimed.push(ScheduleRow { next_run: next, last_run: Some(now_s.clone()), updated_at: now_s.clone(), ..row }); } } tx.commit()?; Ok(claimed) }
}
