...
fn bounded<'a>(value: &'a str, max: usize, field: &str) -> Result<&'a str> {
    if value.trim().is_empty() {
        bail!("credential {field} must not be empty")
    }
    if value.len() > max {
        bail!("credential {field} exceeds maximum length")
    }
    Ok(value)
}
fn key_bytes() -> Result<[u8; 32]> {
    let raw = std::env::var(KEY_ENV)
        .map_err(|_| anyhow!("credential encryption key is not configured"))?;
    if raw.is_empty() {
...
fn key() -> Result<LessSafeKey> {
    ...
    let unbound = UnboundKey::new(&aead::AES_256_GCM, &bytes)
        .map_err(|_| anyhow!("credential encryption key is invalid"))?;
    Ok(LessSafeKey::new(unbound))
}
fn encrypt(secret: &str, key: &LessSafeKey, aad_bytes: &[u8]) -> Result<Vec<u8>> {
...
fn decrypt(mut data: Vec<u8>, key: &LessSafeKey, aad_bytes: &[u8]) -> Result<String> {
    if data.len() < NONCE_LEN + 16 {
        bail!("credential ciphertext is invalid")
    }
    let mut nonce_bytes = [0u8; NONCE_LEN];
...
pub fn encrypt_secret(secret: &str) -> Result<String> {
    let k = key()?;
    let bytes = encrypt(secret, &k, b"standalone_secret")?;
...
pub fn decrypt_secret(encoded: &str) -> Result<String> {
    ...
    decrypt(bytes, &k, b"standalone_secret")
}

fn row(row: &rusqlite::Row<'_>) -> rusqlite::Result<CredentialRow> {
    Ok(CredentialRow {
        id: row.get(0)?,
        name: row.get(1)?,
        kind: row.get(2)?,
...
impl crate::Persistence {
    pub fn store_credential(&self, name: &str, kind: &str, secret: &str) -> Result<CredentialRow> {
        let name = bounded(name, MAX_NAME, "name")?;
        let kind = bounded(kind, MAX_KIND, "kind")?;
        if secret.is_empty() {
        ...
        let aad = format!("{id}:{name}");
        let ciphertext = encrypt(secret, &key, aad.as_bytes())?;
        ...
        let result = tx.query_row(
            "SELECT id,name,kind,created_at,updated_at FROM credentials WHERE name=?1",
            params![name],
            row,
        )?;
        tx.commit()?;
    ...
    pub fn delete_credential(&self, id: &str) -> Result<bool> {
        let id = bounded(id, 128, "id")?;
        let conn = self.conn.lock();
        ensure_schema(&conn)?;
        Ok(conn.execute("DELETE FROM credentials WHERE id=?1", params![id])? != 0)
    }
    pub fn resolve_secret(&self, id: &str) -> Result<Option<String>> {
        let id = bounded(id, 128, "id")?;
        let key = key()?;
        let conn = self.conn.lock();
        ensure_schema(&conn)?;
        let row_data: Option<(String, Vec<u8>)> = conn
            .query_row(
                "SELECT name, ciphertext FROM credentials WHERE id=?1",
                params![id],
                |r| Ok((r.get(0)?, r.get(1)?)),
            )
            .optional()?;
        if let Some((name, ciphertext)) = row_data {
            let aad = format!("{id}:{name}");
            decrypt(ciphertext, &key, aad.as_bytes()).map(Some)
        } else {
            Ok(None)
        }
    }
...
mod tests {
    ...
    fn missing_key_refuses_storage() {
        ...
        assert!(db
            .store_credential("mail", "password", "secret-value")
            .is_err());
    ...
    fn rows_hide_and_encrypt_secret() {
        ...
        let db = crate::Persistence::in_memory().unwrap();
        let first = db
            .store_credential("mail", "password", "secret-one")
            .unwrap();
        let second = db
            .store_credential("mail", "password", "secret-two")
            .unwrap();
        assert_eq!(first.id, second.id);
        assert_eq!(
            db.resolve_secret(&second.id).unwrap().as_deref(),
            Some("secret-two")
        );
