...
fn encrypt(secret: &str, key: &LessSafeKey, aad_bytes: &[u8]) -> Result<Vec<u8>> {
    ...
    key.seal_in_place_append_tag(Nonce::assume_unique_for_key(nonce_bytes), Aad::from(aad_bytes), &mut data).map_err(|_| anyhow!("credential encryption failed"))?;
...
fn decrypt(mut data: Vec<u8>, key: &LessSafeKey, aad_bytes: &[u8]) -> Result<String> {
    ...
    let plain = key.open_in_place(Nonce::assume_unique_for_key(nonce_bytes), Aad::from(aad_bytes), &mut data[NONCE_LEN..]).map_err(|_| anyhow!("credential decryption failed"))?;
...
impl crate::Persistence {
    pub fn store_credential(&self, name: &str, kind: &str, secret: &str) -> Result<CredentialRow> {
        ...
        let result = tx.query_row("SELECT id,name,kind,created_at,updated_at FROM credentials WHERE name=?1", params![name], row)?;
        tx.commit()?;
        Ok(result)
    }
    ...
    pub fn resolve_secret(&self, id: &str) -> Result<Option<String>> {
        ...
        ensure_schema(&conn)?;
        let row_data: Option<(String, Vec<u8>)> = conn.query_row(
