...
use pr_memory::{content_hash, AbsorbFact, AbsorbRequest, GcOptions, Memory, Scope, ScopeFilter};
use serde_json::json;
...
async fn search_all(mem: &Memory, query: &str) -> Vec<(String, f32)> {
    let hits = mem
        .search(query, &ScopeFilter::persistent(), None)
        .await
        .unwrap();
    hits.into_iter()
        .map(|h| (h.memory.content, h.score))
        .collect()
}
...
async fn memory_engineering_full_pipeline() {
    ...
    let r1 = absorb_one(
        &mem,
        "PostgreSQL 16 is fast for complex queries",
        Scope::Agent,
        vec!["database"],
    )
    .await;
    ...
    let r3 = absorb_one(
        &mem,
        "PostgreSQL 17 is even faster than v16 for complex queries",
        Scope::Agent,
        vec!["database"],
    )
    .await;
    ...
    assert!(
        r3.created >= 1,
        "without LLM, updated fact is inserted as new"
    );
    ...
    let r5a = absorb_one(
        &mem,
        "I prefer terse code reviews with no preamble at work",
        Scope::Agent,
        vec!["preferences"],
    )
    .await;
    ...
    let r5b = absorb_one(
        &mem,
        "I prefer detailed documentation with examples for personal projects",
        Scope::Agent,
        vec!["preferences"],
    )
    .await;
    ...
    let r7a = absorb_one(
        &mem,
        "Redis is great for caching session data",
        Scope::Agent,
        vec!["redis"],
    )
    .await;
    ...
    let r7b = absorb_one(
        &mem,
        "Redis works well for caching session data in production",
        Scope::Agent,
        vec!["redis"],
    )
    .await;
    ...
    let redis_facts = mem
        ...
        .unwrap()
        .into_iter()
        .filter(|r| r.tags.contains(&"redis".to_string()))
        .collect::<Vec<_>>();
    ...
    let _ = absorb_one(
        &mem,
        "Python 3.12 has improved error messages with better tracebacks",
        Scope::Agent,
        vec!["python"],
    )
    .await;
    let _ = absorb_one(
        &mem,
        "Python 3.12 type system is much more expressive with PEP 695",
        Scope::Agent,
        vec!["python"],
    )
    .await;

    let python_facts = mem
        .db
        .list(&ScopeFilter::persistent(), Some("active"), 100)
        .unwrap();
    let py1 = python_facts
        .iter()
        .find(|r| r.content.contains("error messages"))
        .unwrap();
    ...
    if hits.len() >= 2 {
        let boosted_idx = hits
            .iter()
            .position(|(c, _)| c.contains("error messages"))
            .unwrap();
        let other_idx = hits
            .iter()
            .position(|(c, _)| c.contains("type system"))
            .unwrap();
    ...
    assert_eq!(
        old_status.as_deref(),
        Some("archived"),
        "low-confidence old memory must be archived by GC"
    );

    // ── 10. Digest — pre-session context ────────────────────────────
    println!("\n▶ 10. Digest — pre-session context load");
    let digest = mem
        .digest("PostgreSQL", &ScopeFilter::persistent())
        .await
        .unwrap();
