...
async fn batch_absorb_consolidates_near_duplicates() {
    ...
    println!(
        "created={} consolidated={} skipped={}",
        report.created, report.consolidated, report.skipped
    );
    assert!(
        report.consolidated >= 2,
        "близкие факты батча должны консолидироваться N→1"
    );
    ...
    assert!(
        active.len() < 3,
        "после консолидации строк меньше, чем фактов: {}",
        active.len()
    );
...
async fn digest_collects_todos_recent_and_relevant() {
    ...
    let digest = mem
        ...
        .unwrap();
    println!(
        "relevant={} todos={} recent={}",
        digest.relevant.len(),
        digest.open_todos.len(),
        digest.recent.len()
    );

    assert!(
        !digest.relevant.is_empty(),
        "должно быть релевантное по запросу"
    );
    assert_eq!(
        digest.open_todos.len(),
        1,
        "только открытый TODO (done — исключён)"
    );
    assert!(
        digest.open_todos[0].content.contains("Review pull request"),
...
async fn entity_graph_multihop_from_absorbed_facts() {
    ...
    let start = mem
        .db
        .entity_by_name("Ivan Petrov", "person")
        .unwrap()
    ...
    assert!(
        paths
            .iter()
            .any(|p| p.nodes.iter().any(|n| n.name == "Moscow")),
...
async fn cyrillic_facts_are_searchable() {
    ...
    assert!(
        !hits.is_empty(),
        "кириллический запрос должен находить русский факт"
    );
    assert!(
        hits[0].memory.content.contains("Акме"),
        "первым должен быть русский факт про Акме, получено: {}",
...
async fn empty_store_search_and_digest_are_safe() {
    let mem = in_memory();
    let hits = mem
        .search("anything", &ScopeFilter::persistent(), None)
        .await
        .unwrap();
    assert!(hits.is_empty(), "пустой стор → пустая выдача");

    let digest = mem
        .digest("anything", &ScopeFilter::persistent())
        .await
        .unwrap();
    assert!(digest.relevant.is_empty() && digest.open_todos.is_empty() && digest.recent.is_empty());

    let block = mem
        .digest_block("anything", &ScopeFilter::persistent(), 1000)
        .await;
    assert!(
        block.is_empty(),
        "digest_block пустого стора возвращает пустую строку"
    );
}
...
async fn follow_latest_and_full_history_resolve_chains() {
    ...
    println!("history: {ids:?}");
    assert!(
        ids.contains(&v1.id.as_str())
            && ids.contains(&v2.id.as_str())
            && ids.contains(&v3.id.as_str()),
...
async fn absorb_metadata_expires_at_directly() {
    ...
    let hits = mem
        .search("temporary fact", &ScopeFilter::persistent(), None)
        .await
        .unwrap();
    assert!(
        hits.is_empty(),
        "истёкший факт не должен находиться поиском"
    );

    // GC архивирует его.
    let report = mem.gc(&pr_memory::GcOptions::default()).await.unwrap();
    assert_eq!(
        report.expired_archived,
        1,
        "GC должен заархивировать истёкший: {}",
        report.summary_line()
    );
}
...
async fn digest_excludes_expired_facts() {
    ...
    let digest = mem
        .digest("quarterly goals", &ScopeFilter::persistent())
        .await
        .unwrap();
    let contents: Vec<&str> = digest
        .relevant
        .iter()
        .map(|h| h.memory.content.as_str())
        .collect();
    println!("relevant: {contents:?}");
    assert!(
        contents.iter().any(|c| c.contains("current detail")),
        "свежий факт в дайджесте"
    );
    assert!(
        !contents.iter().any(|c| c.contains("expired detail")),
        "истёкший факт исключён"
    );
}
...
async fn rebuild_embeddings_after_merge_fixes_stale_vector() {
    ...
    mem.pipeline()
        .absorb(AbsorbRequest {
            facts: vec![fact(
                "The company Acme Corp is based in Moscow and develops CRM software",
            )],
            source: "stale".into(),
            scope: Scope::Agent,
            scope_key: String::new(),
    ...
    let r2 = mem
        ...
        .absorb(AbsorbRequest {
            facts: vec![fact(
                "The company Acme Corp is based in Moscow and develops CRM and ERP software",
            )],
            source: "stale".into(),
            scope: Scope::Agent,
            scope_key: String::new(),
    ...
    let before = mem
        .search("ERP software", &ScopeFilter::persistent(), None)
        .await
        .unwrap();
    ...
    assert!(
        !after.is_empty(),
        "после rebuild поиск ERP должен находить объединённую строку"
    );
    assert!(
        after[0].memory.content.contains("ERP"),
        "найдена строка с ERP"
    );
}
...
async fn search_importance_breaks_ties() {
    ...
    let hits = mem
        .search("Rust backend", &ScopeFilter::persistent(), None)
        .await
        .unwrap();
    assert_eq!(hits.len(), 2);
    assert_eq!(
        hits[0].memory.id, high.id,
