...
impl SupervisorConfig {
    ...
    pub fn from_env() -> Result<Self, SupervisorError> {
        ...
        let token = env::var("COMPUTER_TOKEN")
            .map_err(|_| SupervisorError::MissingConfig("COMPUTER_TOKEN"))?;
        let base_port = env::var("COMPUTER_BASE_PORT")
            .ok()
            .map(|value| {
                value.parse().map_err(|_| {
                    SupervisorError::InvalidConfig("COMPUTER_BASE_PORT must be a valid port")
                })
            })
            .transpose()?
    ...
    fn validate(&self) -> Result<(), SupervisorError> {
        if self.image.trim().is_empty() {
            return Err(SupervisorError::InvalidConfig("image must not be empty"));
        }
        if !valid_docker_component(&self.network) {
            return Err(SupervisorError::InvalidConfig(
                "network contains invalid characters",
            ));
        }
        if self.token.is_empty() {
            return Err(SupervisorError::InvalidConfig("token must not be empty"));
        }
        if self.base_port == 0 {
            return Err(SupervisorError::InvalidConfig("base_port is invalid"));
        }
        if self.container_port == 0 {
            return Err(SupervisorError::InvalidConfig("container_port is invalid"));
        }
        Ok(())
    }
}
...
impl fmt::Debug for ComputerSupervisor {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ComputerSupervisor")
...
impl ComputerSupervisor {
    ...
    pub fn from_env() -> Result<Self, SupervisorError> {
        Self::new(SupervisorConfig::from_env()?)
    }
    ...
    pub async fn ensure(&self, agent_id: &str) -> Result<AgentContainer, SupervisorError> {
        ...
        if let Some(summary) = self.inspect_managed(&names.container).await? {
            ...
            if !running {
                self.with_timeout(
                    self.docker
                        .start_container(&names.container, None::<StartContainerOptions<String>>),
                )
                .await?;
            }
        ...
        bindings.insert(
            ...
            Some(vec![PortBinding {
                ...
                host_port: Some(names.port.to_string()),
            }]),
        );
        ...
        self.with_timeout(self.docker.create_container(
        ...
        Ok(AgentContainer {
            ...
            profile_volume: names.profile_volume,
            port: names.port,
            running: true,
            health: None,
        })
    ...
    pub async fn stop(&self, agent_id: &str) -> Result<(), SupervisorError> {
        ...
        {
            Ok(()) => Ok(()),
            Err(SupervisorError::Docker(bollard::errors::Error::DockerResponseServerError {
                status_code: 404,
                ..
            })) => Ok(()),
            Err(error) => Err(error),
    ...
    pub async fn reset(&self, agent_id: &str) -> Result<(), SupervisorError> {
        ...
        let name = Names::for_agent(id, self.config.base_port)?.container;
        match self
            .with_timeout(self.docker.remove_container(
                &name,
    ...
    async fn inspect_managed(
        &self,
        name: &str,
    ) -> Result<Option<bollard::models::ContainerSummary>, SupervisorError> {
        let mut filters = HashMap::new();
...
impl Names {
    fn for_agent(agent_id: &str, base_port: u16) -> Result<Self, SupervisorError> {
        ...
        let port = base_port
            .checked_add(offset)
            .ok_or(SupervisorError::InvalidConfig("base port range exhausted"))?;
        Ok(Self {
            container: format!("fathom-computer-{hash}"),
            workspace_volume: format!("fathom-workspace-{hash}"),
            profile_volume: format!("fathom-profile-{hash}"),
            port,
        })
    }
}
...
fn valid_docker_component(value: &str) -> bool {
    !value.is_empty()
        && value.len() <= 63
        && value
            .bytes()
...
mod tests {
    ...
    #[test]
    fn validates_ids() {
        assert!(validate_agent_id("agent-1.alpha").is_ok());
        assert!(validate_agent_id("").is_err());
        assert!(validate_agent_id("../escape").is_err());
        assert!(validate_agent_id("agent/name").is_err());
    }
    #[test]
    fn names_are_deterministic_and_private() {
        let a = Names::for_agent("agent-1", 19000).unwrap();
        let b = Names::for_agent("agent-1", 19000).unwrap();
        assert_eq!(a.container, b.container);
        assert!(!a.container.contains("agent-1"));
        assert!(a.port >= 19000);
    }
}
