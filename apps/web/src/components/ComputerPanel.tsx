...
export default function ComputerPanel() {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#080909]">
      <header className="border-b border-white/[0.08] px-5 py-5 md:px-8"><p className="ops-kicker">Computer / remote workspace</p><div className="mt-1 flex flex-wrap items-center justify-between gap-3"><h1 className="text-xl tracking-tight text-gray-100">Remote computer workspace</h1><span className={`ops-status shrink-0 ${offline ? 'ops-status-deny' : 'ops-status-allow'}`} aria-live="polite">{offline ? 'OFFLINE' : loading ? 'CONNECTING' : 'READY'}</span></div><p className="mt-1 max-w-xl text-xs text-gray-500">A governed remote computer surface. Take control before issuing actions; screenshots are read-only until then.</p></header>
      <div className="grid gap-5 p-5 md:p-8 xl:grid-cols-[minmax(0,1fr)_300px]">
        <section className="ops-panel min-h-[500px]">
          <div className="ops-panel-head"><div><p className="ops-kicker">{session ? 'Active browser session' : 'No active session'}</p><h2>Viewport</h2></div><button type="button" onClick={() => void refresh()} disabled={working} className="ops-button-secondary">↻ Snapshot</button></div>
          <form className="mb-4 flex flex-col gap-2 sm:flex-row" onSubmit={event => { event.preventDefault(); if (!session || !controlled || working) return; void run(() => api.computers.navigate(url)) }}><label className="sr-only" htmlFor="computer-url">Destination URL</label><input id="computer-url" type="text" inputMode="url" className="ops-input min-w-0 flex-1 font-mono" value={url} onChange={e => setUrl(e.target.value)} placeholder="https://example.com" /><button type="submit" disabled={!controlled || working || !session} className="ops-button-secondary">Navigate</button></form>
