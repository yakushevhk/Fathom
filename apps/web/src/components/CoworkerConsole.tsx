...
export default function CoworkerConsole() {
  ...
  const createSchedule = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setScheduleDraft(current => ({ ...current, query: '' })); setNotice('Schedule saved')
      ...
      <div className="grid gap-5 p-5 md:p-8 xl:grid-cols-[minmax(230px,.75fr)_minmax(0,1.45fr)]">
        <aside className="space-y-5">
          ...
          <section className="ops-panel">
            ...
            {loading ? <div className="ops-empty">Loading schedules…</div> : schedules.length === 0 ? <div className="ops-empty">No recurring worker schedules yet. Add one to save recurring work for a scheduler claim.</div> : <div className="max-h-72 space-y-3 overflow-y-auto pr-1">{schedules.map(schedule => <article key={schedule.id} className="border-l border-white/[0.12] pl-3"><div className="flex items-center justify-between gap-2"><code className="text-[11px] text-gray-300">{schedule.cron_expression}</code><div className="flex items-center gap-2"><span className={`text-[9px] uppercase tracking-wider ${schedule.enabled ? 'text-emerald-400' : 'text-gray-600'}`}>{schedule.enabled ? 'enabled' : 'paused'}</span><button type="button" onClick={async () => { if (confirm('Delete this schedule?')) { await api.schedules.delete(schedule.id); setSchedules(s => s.filter(item => item.id !== schedule.id)) } }} className="text-gray-500 hover:text-red-400 text-xs transition-colors" title="Delete schedule">✕</button></div></div><p className="mt-1 line-clamp-2 text-[11px] text-gray-500">{schedule.query}</p><p className="mt-1 font-mono text-[9px] text-gray-700">next {new Date(schedule.next_run).toLocaleString()} · {schedule.timezone}</p></article>)}</div>}
          </section>
        </aside>
        <div className="space-y-5">
          ...
          <section className="ops-panel">
            ...
            <form onSubmit={createSchedule} className="space-y-3"><div className="grid gap-3 sm:grid-cols-3"><label className="ops-label">Worker<select required className="ops-input mt-1" value={scheduleDraft.coworker_id} onChange={e => setScheduleDraft(current => ({ ...current, coworker_id: e.target.value }))}><option value="">Select worker</option>{coworkers.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="ops-label">Cron expression<input required maxLength={256} className="ops-input mt-1 font-mono" value={scheduleDraft.cron_expression} onChange={e => setScheduleDraft(current => ({ ...current, cron_expression: e.target.value }))} placeholder="0 9 * * 1" /></label><label className="ops-label">Timezone<input required maxLength={128} className="ops-input mt-1" value={scheduleDraft.timezone} onChange={e => setScheduleDraft(current => ({ ...current, timezone: e.target.value }))} placeholder="UTC" /></label></div><label className="ops-label">Run query<textarea required maxLength={20000} rows={3} className="ops-input mt-1 resize-y" value={scheduleDraft.query} onChange={e => setScheduleDraft(current => ({ ...current, query: e.target.value }))} placeholder="What should this worker check on each run?" /></label><div className="flex items-center justify-between gap-3"><label className="flex items-center gap-2 text-xs text-gray-400"><input type="checkbox" checked={scheduleDraft.enabled} onChange={e => setScheduleDraft(current => ({ ...current, enabled: e.target.checked }))} className="accent-[#c0d4ce]" />Store schedule as enabled</label><button type="submit" className="ops-button-primary" disabled={scheduleSaving || !scheduleDraft.coworker_id || !scheduleDraft.query.trim()}>{scheduleSaving ? 'Saving…' : 'Save schedule'}</button></div></form>
          </section>
        </div>
      </div>
    </div>
