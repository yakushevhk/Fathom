  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3">
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="ops-label">Name<input required disabled={readOnly} maxLength={100} className="ops-input mt-1" value={draft.name} onChange={e => onChange({ ...draft, name: e.target.value })} placeholder="Operations coordinator" /></label>
        <label className="ops-label">Title<input disabled={readOnly} maxLength={200} className="ops-input mt-1" value={draft.title} onChange={e => onChange({ ...draft, title: e.target.value })} placeholder="Worker operations" /></label>
      </div>
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="ops-label">Role<input disabled={readOnly} maxLength={100} className="ops-input mt-1" value={draft.role} onChange={e => onChange({ ...draft, role: e.target.value })} placeholder="Operations worker" /></label>
      ...
      <label className="ops-label">System prompt<textarea required disabled={readOnly} maxLength={20000} rows={5} className="ops-input mt-1 resize-y" value={draft.prompt} onChange={e => onChange({ ...draft, prompt: e.target.value })} placeholder="Describe how this autonomous worker should operate…" /></label>
      <label className="flex items-center gap-2 text-xs text-gray-400"><input type="checkbox" disabled={readOnly} checked={draft.active} onChange={e => onChange({ ...draft, active: e.target.checked })} className="accent-[#c0d4ce]" />Available for new tasks</label>
      <div className="flex justify-end gap-2 pt-1">
        {editing && <button type="button" onClick={onCancel} className="ops-button-secondary">Cancel</button>}
        {!readOnly && <button type="submit" disabled={saving || !draft.name.trim() || !draft.prompt.trim()} className="ops-button-primary">{saving ? 'Saving…' : editing ? 'Save changes' : 'Create worker'}</button>}
      </div>
    </form>
  )
}
    ...
    <div className="flex-1 overflow-y-auto bg-[#080909]">
      <header className="border-b border-white/[0.08] px-5 py-5 md:px-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><p className="ops-kicker">Work / autonomous worker fleet</p><h1 className="mt-1 text-xl tracking-tight text-gray-100">Coworkers &amp; recurring operations</h1><p className="mt-1 max-w-2xl text-xs text-gray-500">Configure autonomous workers, route their conversations, and keep recurring operations on time.</p></div>
      ...
      <div className="grid gap-5 p-5 md:p-8 xl:grid-cols-[minmax(230px,.75fr)_minmax(0,1.45fr)]">
        ...
        <div className="space-y-5">
          ...
          <section className="ops-panel">
            <div className="ops-panel-head"><div><p className="ops-kicker">Automation</p><h2>Schedule a recurring operation</h2></div></div>
            <form onSubmit={createSchedule} className="space-y-3"><div className="grid gap-3 sm:grid-cols-3"><label className="ops-label">Worker<select required className="ops-input mt-1" value={scheduleDraft.coworker_id} onChange={e => setScheduleDraft(current => ({ ...current, coworker_id: e.target.value }))}><option value="">Select worker</option>{coworkers.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="ops-label">Cron expression<input required maxLength={256} className="ops-input mt-1 font-mono" value={scheduleDraft.cron_expression} onChange={e => setScheduleDraft(current => ({ ...current, cron_expression: e.target.value }))} placeholder="0 9 * * 1" /></label><label className="ops-label">Timezone<input required maxLength={128} className="ops-input mt-1" value={scheduleDraft.timezone} onChange={e => setScheduleDraft(current => ({ ...current, timezone: e.target.value }))} placeholder="UTC" /></label></div><label className="ops-label">Run query<textarea required maxLength={20000} rows={3} className="ops-input mt-1 resize-y" value={scheduleDraft.query} onChange={e => setScheduleDraft(current => ({ ...current, query: e.target.value }))} placeholder="What should this worker check on each run?" /></label><div className="flex items-center justify-between gap-3"><label className="flex items-center gap-2 text-xs text-gray-400"><input type="checkbox" checked={scheduleDraft.enabled} onChange={e => setScheduleDraft(current => ({ ...current, enabled: e.target.checked }))} className="accent-[#c0d4ce]" />Start schedule enabled</label><button type="submit" className="ops-button-primary" disabled={scheduleSaving || !scheduleDraft.coworker_id || !scheduleDraft.query.trim()}>{scheduleSaving ? 'Scheduling…' : 'Create schedule'}</button></div></form>
          </section>
        </div>
      </div>
    </div>
