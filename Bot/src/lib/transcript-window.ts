...
/** Sliding window expansion: bounds maximum rendered messages to avoid DOM bloating */
export function expandSlidingWindowStart(
  startIndex: number,
  size: number = TRANSCRIPT_WINDOW_SIZE,
  endIndex: number | null = null,
): { start: number; end: number | null } {
  const nextStart = Math.max(0, startIndex - size);
  if (endIndex !== null && endIndex - nextStart > MAX_TRANSCRIPT_MOUNTED) {
    return { start: nextStart, end: nextStart + MAX_TRANSCRIPT_MOUNTED };
  }
  return { start: nextStart, end: endIndex };
}
