...
export function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <SidebarDrawerContext.Provider value={onClose}>
    <aside
      ref={sidebarRef}
      tabIndex={-1}
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-hairline/30 px-3.5 pt-[calc(0.625rem+env(safe-area-inset-top,0px))] pb-2.5 md:hidden">
      ...
      {/* Footer */}
      <div className={cn("pt-2 pb-[max(0.75rem,env(safe-area-inset-bottom,0px))]", density === "icons" ? "px-2" : "px-3")}>
        {density === "icons" && (
          <>
          <button
