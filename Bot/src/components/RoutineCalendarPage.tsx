...
function EventEditor({
  ...
  return (
    <div className="modal-backdrop fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-label={existingRoutine || existingCall ? "Edit calendar event" : "Create calendar event"} tabIndex={-1} className="animate-pop-in max-h-[94vh] w-full max-w-[760px] overflow-y-auto rounded-2xl border border-hairline/60 bg-panel shadow-2xl">
            ...
            <div className="min-w-0 flex-1 space-y-3">
              {recurrence !== "interval" && (
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {(kind === "call" || recurrence === "none" || recurrence === "weekly") && <input type="date" value={date} onChange={(event) => setDate(event.target.value)} className="rounded-lg border border-hairline/50 bg-inset px-3 py-2 text-[13px] text-ink outline-none focus:border-accent" style={{ colorScheme: "var(--code-color-scheme)" }} />}
                  <input type="time" step={CALENDAR_SLOT_MINUTES * 60} value={startTime} onChange={(event) => setStartTime(event.target.value)} className="rounded-lg border border-hairline/50 bg-inset px-3 py-2 text-[13px] text-ink outline-none focus:border-accent" style={{ colorScheme: "var(--code-color-scheme)" }} />
                  {kind === "call" && <>
              ...
              <div className="min-w-0 flex-1">
                {isRoomGoal ? (
                  <div className="rounded-xl border border-accent/35 bg-accent/[0.07] p-3">
                    ...
                    <div className="mt-1 text-[11px] leading-relaxed text-ink-secondary">Parallel keeps the group and its member hand-offs together for the full goal.</div>
                  </div>
