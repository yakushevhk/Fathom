...
1. Quit every Parallel window.
2. Confirm no old Parallel process remains.
3. Start the app once and wait for the local harness.
4. If it still fails, inspect the desktop log.

On macOS, packaged logs are under `~/Library/Logs/Parallel`. On Windows they are under the Parallel log directory in `%APPDATA%`.
...
Developers can set `OMB_DATA_DIR` to run against a clean Parallel data directory. This is useful for reproduction and screenshots without touching a real workspace.
