...
function requireUpdaterTarget(resources, label) {
  ...
  if (!/^owner: milind-soni$/m.test(update) || !/^repo: Parallel$/m.test(update)) {
    fail(`${label} app-update.yml does not point at milind-soni/Parallel`);
  }
}
...
try {
  ...
  const debAppRoot = path.join(extracted, "opt", "Parallel");
  ...
  for (const expected of [
    "Name=Parallel",
    "Exec=/opt/Parallel/openmausbot %U",
    "Icon=openmausbot",
