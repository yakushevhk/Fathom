...
<head>
  ...
  <title>Parallel — Cua Driver third-party licenses</title>
...
Parallel redistributes two executables from the official Cua Driver ${RELEASE_VERSION} Linux x64 release: \`cua-driver\` and \`cua-cursor-theme\`.
...
The complete MPL-2.0 text and package copyright notices appear in \`THIRD_PARTY_LICENSES.html\`. These files remain under their original licenses; Parallel's and Cua's MIT licenses do not replace them.
