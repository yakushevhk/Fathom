...
try {
  ...
  if (status.startsWith("ii")) fail("refusing to replace a pre-existing Parallel installation");
} catch (error) {
...
const legacyApp = path.join(legacyRoot, "opt", "Parallel");
...
try {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "Version: 0.1.7",
      "Architecture: amd64",
      "Maintainer: Parallel CI <ci@openmausbot.invalid>",
      "Description: Legacy Parallel directory-mode upgrade fixture",
      "",
    ].join("\n"),
    { mode: 0o644 },
  );
  ...
  for (const directory of ["/opt/Parallel", "/opt/Parallel/resources"]) {
  ...
  for (const directory of [
    "/opt/Parallel",
    "/opt/Parallel/resources",
    "/opt/Parallel/resources/cua-linux-x64",
  ]) {
  ...
  for (const executable of ["cua-driver", "cua-cursor-theme"]) {
    const file = path.join("/opt/Parallel/resources/cua-linux-x64", executable);
  ...
  const chromiumSandbox = "/opt/Parallel/chrome-sandbox";
