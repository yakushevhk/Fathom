...
for (const appName of ["openmausbot", "Parallel"]) {
...
try {
  ...
  if (!String(title).includes("Parallel")) throw new Error(`unexpected renderer title: ${title}`);
  ...
  if (sessionBlocked) {
    ...
    const activeUserData = ["openmausbot", "Parallel"]
    ...
    const userData = ["openmausbot", "Parallel"]
      .map((name) => path.join(xdgConfig, name))
      .find((directory) => existsSync(path.join(directory, "cua-connection.json")));
