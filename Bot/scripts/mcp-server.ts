#!/usr/bin/env node
// Model Context Protocol (MCP) Server for Parallel
...
export function validateBaseUrl(url: string): string {
  ...
  try {
    ...
    throw new Error(`Invalid Parallel URL: '${url}'`);
  }
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    throw new Error("Parallel URL must use http:// or https://");
  }
  if (parsed.username || parsed.password) {
    throw new Error("Parallel URL must not contain credentials; use OPENMAUSBOT_TOKEN instead");
  }
  if ((parsed.pathname !== "/" && parsed.pathname !== "") || parsed.search || parsed.hash) {
    throw new Error("Parallel URL must be an origin without a path, query, or fragment");
  }
...
async function fetchJson(url: string, options: RequestInit = {}): Promise<any> {
  ...
  if (!response.ok) {
    ...
    if (response.status === 403 && !process.env.OPENMAUSBOT_TOKEN?.trim()) {
      throw new Error(
        "Parallel refused this write because the installed desktop app requires a paired session token. " +
  ...
  try {
    ...
    throw new Error(`Parallel API returned a non-JSON response from ${url}`);
  }
}
...
export async function probeBaseUrls(candidates: string[]): Promise<string> {
  ...
  for (const unvalidated of candidates) {
    ...
    try {
      ...
      if (health?.app !== "openmausbot") {
        failures.push(`${candidate} answered, but it was not Parallel`);
        continue;
      }
      return candidate;
    } catch (error) {
  ...
  throw new Error(`Could not find a running Parallel server. ${failures.join("; ")}`);
}
...
export const TOOLS: McpToolDefinition[] = [
  {
    ...
    description: "Check whether the Parallel server is reachable.",
    inputSchema: {
      type: "object",
      properties: {},
...
function idArg(args: Record<string, unknown>, key: string): string {
  ...
  if (!/^[\w-]+$/.test(value)) throw new ToolInputError(`${key} is not a valid Parallel ID`);
  return value;
}
...
export async function handleToolCall(
  ...
  switch (name) {
    case "get_system_health": {
      ...
      if (res?.app !== "openmausbot") throw new Error("The configured endpoint is not an Parallel server");
      return {
        status: "connected",
    ...
    case "create_bot": {
      ...
      if (!isRecord(created?.bot) || typeof created.bot.id !== "string") {
        throw new Error("Parallel did not return the created bot");
      }
    ...
    case "update_bot_profile": {
      ...
      if (!isRecord(result?.bot)) {
        throw new Error("Parallel did not return the updated bot");
      }
    ...
    case "create_channel": {
      ...
      if (!isRecord(created?.group) || typeof created.group.id !== "string") {
        throw new Error("Parallel did not return the created channel");
      }
    ...
    case "update_channel": {
      ...
      if (!isRecord(result?.group)) {
        throw new Error("Parallel did not return the updated channel");
      }
    ...
    case "create_task": {
      ...
      if (!isRecord(result?.task) || typeof result.task.threadId !== "string") {
        throw new Error("Parallel did not return the created task");
      }
    ...
    case "rename_task": {
      ...
      if (!isRecord(result?.task)) {
        throw new Error("Parallel did not return the renamed task");
      }
      return {
        success: true,
        targetType: args.target_type,
    ...
    case "set_bot_model": {
      ...
      if (args.task_id !== undefined) {
        ...
        if (!isRecord(res?.task)) throw new Error("Parallel did not return the updated task");
  ...
  try {
    ...
    if (method === "initialize") {
      ...
      return formatResponse(id, {
        ...
        instructions: "Use bounded read tools before mutating the Parallel team. Approval grants, deletion, and computer lifecycle are intentionally unavailable.",
      });
    }
...
if (process.argv[1] && (process.argv[1].endsWith("mcp-server.ts") || process.argv[1].endsWith("mcp-server.js"))) {
  ...
  log("Parallel MCP server running on stdio");
}
