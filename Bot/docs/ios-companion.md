...
The iOS app is a thin, native client for the Parallel instance running on
...
an Parallel account of its own.
...
The Mac must be running Parallel and must not be asleep. Desktop
...
to reach. Parallel then places the computer's MagicDNS name in that
...
Tailscale is optional. The direct path does not use an Parallel-operated
...
The Parallel control plane stores account and installation metadata plus
...
The Mac must remain awake with Parallel running for chats, approvals, and
...
An Parallel account is not required for nearby, manual, or Tailscale
