...
Install [Node.js](https://nodejs.org/) 24 or newer, then choose either way to run Parallel:
...
Use the same command next time. The first launch guides you through setup; later launches reuse your saved AI connection and phone-access choice. `openmausbot start` is the same as the bare command. If that workspace is already running, Parallel opens it instead of starting a second server.
...
- **Managed HTTPS address:** setup asks explicit permission for a public endpoint through Cloudflare and a possible connector download. Device pairing protects chat and settings; the pairing page and basic server identity remain publicly reachable. Sign in to an **Parallel account** using an emailed code, or reuse this machine's saved account. This account is separate from ChatGPT, Claude, or an API-provider account. The connection stays active while Parallel runs.
...
- **iPhone/iPad:** scan the QR with Camera to open Safari. If you already have the Parallel iOS app, use its pairing scanner or paste the full link there.
...
| `openmausbot login` | Sign in to an Parallel account for `--tunnel`; this does not sign in to an AI provider or start the tunnel. |
