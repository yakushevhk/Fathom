...
Parallel can turn a Linux server you already own into a bot's computer. The agent process stays on your
...
only surface exposed: Parallel never opens a public port on the VPS, never stores your SSH key or
...
Parallel connects only through a named alias in your `~/.ssh/config` — you type the alias into
...
**Host key first, by hand.** Connect once manually before pointing Parallel at the alias:
...
- **No public ports.** The managed container is created with no published ports, and Parallel refuses to
...
- **Firewall the VPS to SSH only**, ideally from your IP. Nothing Parallel does needs any other inbound
  port open, so anything else open is pure attack surface.
- **Nothing sensitive is stored.** The only thing Parallel persists is the alias name itself
...
- **Remove** is yours, done by hand when a bot no longer needs the server:
  `docker -H ssh://my-vps rm -f <container>`. Parallel never deletes a container on its own.
