...
The Parallel desktop app includes a local stdio MCP server. It lets another MCP client coordinate your
Parallel team while the desktop app and its harness are running.
...
Start Parallel, then configure the MCP client to run:
...
```json
{
  "mcpServers": {
    "openmausbot": {
      "command": "pnpm",
      "args": ["--dir", "/absolute/path/to/Parallel", "mcp"]
    }
  }
}
```
...
1. In Parallel, open **Settings → Phone → Set up a phone** and reveal the
...
```json
{
  "mcpServers": {
    "openmausbot": {
      "command": "/Applications/Parallel.app/Contents/MacOS/Parallel",
      "args": ["/Applications/Parallel.app/Contents/Resources/server/mcp-server.js"],
...
On Windows, use the installed `Parallel.exe` as `command`, the adjacent
`resources\\server\\mcp-server.js` as the argument, and the same `ELECTRON_RUN_AS_NODE=1` environment value.
The usual per-user install is under `%LOCALAPPDATA%\\Programs\\Parallel`.

On Ubuntu `.deb` installs, the executable is normally `/opt/Parallel/openmausbot` and the script is
`/opt/Parallel/resources/server/mcp-server.js`. Use the same environment value.

## Connection discovery

With no configuration, the MCP process probes Parallel's three desktop ports (`8799`, `18799`, and `28799`)
and accepts only a health response that identifies itself as Parallel. This handles the desktop's normal
