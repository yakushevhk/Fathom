...
Parallel has an Ubuntu 24.04 LTS x86_64 desktop beta. The Electron package embeds the harness server, so
...
- The native Electron window and embedded Parallel server on GNOME Xorg and GNOME Wayland.
...
progress in [issue #29](https://github.com/milind-soni/Parallel/issues/29) and the safety hold in
[issue #345](https://github.com/milind-soni/Parallel/issues/345). Bundled
CUA supply-chain work is tracked in [issue #113](https://github.com/milind-soni/Parallel/issues/113). Xorg is tracked in
[issue #79](https://github.com/milind-soni/Parallel/issues/79), and guarded GNOME/Wayland support in
[issue #109](https://github.com/milind-soni/Parallel/issues/109).

## Download packages

Choose one Ubuntu 24.04 x86_64 package from the latest release:

- [Debian package (`Parallel-amd64.deb`)](https://github.com/milind-soni/Parallel/releases/latest/download/Parallel-amd64.deb) — recommended; APT installs its desktop dependencies and configures the bundled bot browser's sandbox.
- [Portable AppImage (`Parallel.AppImage`)](https://github.com/milind-soni/Parallel/releases/latest/download/Parallel.AppImage) — does not install system files.
- [SHA-256 checksums](https://github.com/milind-soni/Parallel/releases/latest/download/SHA256SUMS-ubuntu-x64.txt)

Versioned packages and previous releases remain available on the
[releases page](https://github.com/milind-soni/Parallel/releases).
...
```sh
git clone https://github.com/milind-soni/Parallel.git
cd Parallel
corepack enable
...
- `release/Parallel-<version>-amd64.deb`
- `release/Parallel-<version>-x86_64.AppImage`
...
```sh
sudo apt install ./Parallel-amd64.deb
```

Then open **Parallel** from the GNOME application launcher. To remove it:
...
```sh
chmod +x release/Parallel-*-x86_64.AppImage
./release/Parallel-*-x86_64.AppImage
```

For a downloaded release AppImage, use `Parallel.AppImage` in place of the versioned path above.
...
Applications launched from GNOME do not inherit the same interactive shell `PATH` as a terminal. Parallel
...
```sh
OMB_EXTRA_PATH=/your/custom/bin ./release/Parallel-*-x86_64.AppImage
```

Restart Parallel after installing or signing in to a CLI.
...
24.04 GNOME Wayland session. Parallel detects Wayland before XWayland when both `WAYLAND_DISPLAY` and
...
automatically. Parallel does not capture screen audio, remember the selected monitor after restart, or
...
starts with `--no-overlay`, so Cua's decorative full-screen X11 cursor surface is never created. Parallel also
...
The Xorg runtime uses private sockets, standard permission mode, per-action Parallel approvals,
...
then runs the same smoke against `/opt/Parallel/openmausbot`. These lanes prove the embedded server and UI are
...
Run the CLI directly in a terminal, finish its sign-in flow, then restart Parallel. If it lives outside the
...
```sh
chmod +x Parallel-*-x86_64.AppImage
file Parallel-*-x86_64.AppImage
```
