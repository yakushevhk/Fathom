...
Cursor is an optional Parallel engine. Parallel runs the official
...
3. Confirm `cursor-agent --version` works. The binary installs to `~/.local/bin` by
   default; Parallel already looks there when launched from a GUI.
...
CLI also implements ACP `session/set_model`, Parallel pins the same id over
...
does not provide a bot approval level. Parallel app turns always provide
one: both **Ask for approval** and **Approve for me** launch Cursor without
`--force`, then Parallel handles its permission requests according to the
bot's current level.
