...
provider's own permission modes, passed through. Parallel does not judge an
...
| **Full access** | Enables the provider's permissive mode for commands, edits, and selected-computer actions, including potentially destructive or sensitive work. Residual native prompts are answered for you. Applies to this bot's direct, scheduled, and delegated work (ask_bot, delegate_bot); delegation uses the receiving bot's setting, never the sender's. Questions and separate Parallel confirmations still wait for you. |
| **Custom (`config.toml`)** | Codex only. Parallel reads and reapplies the effective approval and sandbox settings from your Codex configuration. |
...
or Parallel's separate confirmations for credentials, routines, skills, and
peer communication.
...
ends. Parallel keeps no standing grant for a provider's tool. It is not
...
Native modes require a CLI version that supports them; Parallel does not
silently substitute unrestricted access when a mode is rejected.

### Read-only integration tools

Parallel's built-in agents MCP server describes nine scoped reads with
...
provider's session. Its quieter default is Full access; Parallel keeps its
