...
//    EmbeddedCuaDriverHost so TCC grants attribute to Parallel and the
//    driver inherits them. One prompt, named Parallel, out of the box.
...
async function attachStandalone(signal) {
  ...
  if (!(await socketAlive(STANDALONE_SOCKET))) {
    ...
    // machine already granted — instead of the freshly signed Parallel.
...
async function startEmbedded(binary, signal) {
  ...
  // Parallel rather than to a terminal or helper process.
  ...
  if (!sdk.hasRequiredMacOSPermissions(permissionStatus)) {
    ...
    throw new Error(`${missing || "macOS permissions"} required; grant access in System Settings and restart Parallel`);
  }
