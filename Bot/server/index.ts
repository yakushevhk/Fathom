...
function finishGroupTurnOperation(groupId: string, operation: GroupTurnOperation) {
  ...
  const group = store.group(groupId);
  if (group) broadcast({ kind: "group", group: publicGroupState(group) });
  // Only drain queued channel sends if the operation was NOT cancelled by the user
...
function serializeRoomContext(
  ...
  return messages
    .filter((m) => (m.kind === "text" && m.text) || m.kind === "activity")
    .slice(-GROUP_CONTEXT_MESSAGES)
    .map((m) => {
      if (m.kind === "activity") {
        const actor = m.from?.name ? peerName(m.from.name) : "System";
        const actSummary = m.tool?.name || m.text || "performed an action";
        return `[Activity by ${actor}: ${actSummary}]`;
      }
