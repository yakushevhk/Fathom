...
describe("Box create idempotency", () => {
  ...
  it("bounds retries when a lock disappears between EEXIST and owner read", async () => {
    ...
    try {
      ...
      expect(result.error).toMatch(/locked by another Parallel process/i);
  ...
  it("never bypasses a live elected reaper when contenders race", async () => {
    ...
    try {
      ...
      expect(firstResult.error).toMatch(/locked by another Parallel process/i);
      expect(secondResult.error).toMatch(/locked by another Parallel process/i);
