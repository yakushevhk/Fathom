...
const ROUTINE_FIELDS_SCHEMA = {
  ...
  run_on: {
    type: "string",
    enum: ["maus", "cloud"],
    description: "Where the routine runs. Defaults to maus (this Parallel setup).",
  },
  timeout_minutes: {
    type: "integer",
    minimum: 5,
...
const TOOLS = [
  {
    name: "list_bots",
    description:
      "List the other bots (agents) in your Parallel section, with their model and whether they're busy. Call this before delegate_bot or ask_bot to discover who's available. Use delegate_bot for assignments; use ask_bot only for a short consultation needed inline.",
  ...
  },
  {
    ...
    description:
      "Ask the user for a supported API key through Parallel's secure credential flow. The desktop app and a freshly QR-paired mobile app show a secure entry card; older mobile pairings show how to pair again or finish on the computer. Never claim a secure field opened unless this request succeeds, and never ask the user to paste a secret into chat. The secret is saved by the desktop app and is never returned to you. After calling this tool, end the turn; Parallel resumes the task after the user saves or declines.",
    inputSchema: {
      type: "object",
      properties: {
        credential_id: {
