...
export function claudeCliUpdate(version: string | null, cli: string): ProviderSnapshot["update"] | undefined {
  ...
  return {
    ...
    message:
      ...
      "no compaction window picked by Parallel" +
...
const QUESTION_TIMEOUT_NOTE = "Parallel: nobody answered in time. Use your best judgment and continue.";
const DUPLICATE_ASK_ID_NOTE = "Parallel: duplicate ask id — skipping this request.";
...
function systemEndedReply(kind: Ask["kind"]): { behavior: AskBehavior; message: string } {
  return kind === "question"
    ? { behavior: "answer", message: "Parallel: the turn is ending — wrap up." }
    : { behavior: "deny", message: "Parallel: the turn ended" };
}
  ...
  async create(input: DriverCreateInput<ClaudeConfig>): Promise<ProviderInstance> {
    ...
    const sendTurn = async (turn: SendTurnInput) => {
      ...
      if (turn.integrations?.computer) {
        ...
        // Host tools always route through Parallel's permission broker.
