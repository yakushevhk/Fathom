...
login   signs this machine in to an Parallel account (an emailed code)
...
export async function runPair(options: CliOptions): Promise<number> {
  if (!(await serverUp(options.port))) {
    console.error(`no Parallel server on http://127.0.0.1:${options.port}; start one with \`openmausbot serve\` or set OMB_PORT`);
    return 1;
  }
...
export async function runSessions(options: CliOptions): Promise<number> {
  if (!(await serverUp(options.port))) {
    console.error(`no Parallel server on http://127.0.0.1:${options.port}`);
    return 1;
  }
  if (options.revoke) {
...
export async function runStatus(options: CliOptions, io: CliIo = defaultIo()): Promise<number> {
  ...
  try {
    ...
    io.log(options.json ? JSON.stringify(body, null, 2) : `${body.label} · Parallel ${body.version} on ${body.platform} · id ${body.environmentId}`);
  } catch {
    io.error(`no Parallel server on http://127.0.0.1:${options.port}`);
    code = 1;
  }
  if (!options.json) {
...
export async function runLogin(options: CliOptions, io: CliIo = defaultIo()): Promise<number> {
  ...
  const email = (options.email ?? (await io.ask("Email for your Parallel account: "))).trim();
  if (!email) {
...
export async function runServe(options: CliOptions, log: (line: string) => void = console.log): Promise<number> {
  ...
  try {
    ...
    if (exited !== null) {
      if (exited !== 0) log(`Parallel could not start.${logPath ? ` Details: ${logPath}` : " See the output above."}`);
      return exited;
    }
    if (stopping) return await childExit;
    if (!(await serverUp(options.port, child.pid))) {
      console.error(`Parallel did not become ready within a minute.${logPath ? ` Details: ${logPath}` : " See its output above."}`);
      await stop();
      return 1;
    }
    ...
    log("");
    log(`Parallel is running on http://127.0.0.1:${options.port}${publicUrl ? `, reachable at ${publicUrl}` : ""}`);
    if (options.guided) {
