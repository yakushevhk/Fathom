 ...
 * space (SHOT_WIDTH = 1280), preserving aspect ratio (height scales by W / SHOT_WIDTH).
 * Coordinates are mapped to real display geometry ($W x $H). */
function scaled(varName: string, value: number, axis: "x" | "y" = "x"): string {
  const v = Math.round(value);
  if (axis === "y") {
    return `if [ "$W" -gt ${SHOT_WIDTH} ] 2>/dev/null; then ${varName}=$(( ${v} * W / ${SHOT_WIDTH} )); if [ "$H" -gt 0 ] && [ "$${varName}" -ge "$H" ]; then ${varName}=$(( H - 1 )); fi; else ${varName}=${v}; fi`;
  }
  return `if [ "$W" -gt ${SHOT_WIDTH} ] 2>/dev/null; then ${varName}=$(( ${v} * W / ${SHOT_WIDTH} )); if [ "$${varName}" -ge "$W" ]; then ${varName}=$(( W - 1 )); fi; else ${varName}=${v}; fi`;
}
...
const TOOLS = [
  ...
  },
  {
    name: "drag",
    description:
      "Click and drag from (from_x, from_y) to (to_x, to_y) and return the resulting screen. Useful for moving windows, selecting text, or adjusting sliders.",
    inputSchema: {
      type: "object",
      properties: {
        from_x: { type: "number" },
...
function actionShell(a: any): string | { error: string } {
  ...
  if (kind === "click") {
    ...
    return `${scaled("CX", x, "x")}; ${scaled("CY", y, "y")}; CUA_ARGS=${args}; ${cuaOrX11("click", '"$CUA_ARGS"', fallback)}`;
  }
  if (kind === "drag") {
    const fx = Math.round(Number(a.from_x));
    const fy = Math.round(Number(a.from_y));
    const tx = Math.round(Number(a.to_x));
    const ty = Math.round(Number(a.to_y));
    if (!Number.isFinite(fx) || !Number.isFinite(fy) || !Number.isFinite(tx) || !Number.isFinite(ty)) {
      return { error: "drag needs numeric from_x, from_y, to_x, to_y" };
    }
    const btn = a.button === "right" ? 3 : 1;
    const button = a.button === "right" ? "right" : "left";
    const fallback = `xdotool mousemove $FX $FY mousedown ${btn} mousemove --sync $TX $TY mouseup ${btn}`;
    const args = `$(printf '{"from_x":%s,"from_y":%s,"to_x":%s,"to_y":%s,"button":"${button}","scope":"desktop","session":"${REMOTE_CUA_SESSION}"}' "$FX" "$FY" "$TX" "$TY")`;
    return `${scaled("FX", fx, "x")}; ${scaled("FY", fy, "y")}; ${scaled("TX", tx, "x")}; ${scaled("TY", ty, "y")}; CUA_ARGS=${args}; ${cuaOrX11("drag", '"$CUA_ARGS"', fallback)}`;
  }
  if (kind === "type_text") {
  ...
  if (kind === "scroll") {
    ...
    const hasCoords = Number.isFinite(Number(a.x)) && Number.isFinite(Number(a.y));
    const sx = hasCoords ? Math.round(Number(a.x)) : null;
    const sy = hasCoords ? Math.round(Number(a.y)) : null;

    const moveFallback = hasCoords ? `xdotool mousemove $SX $SY; ` : "";
    const fallback = `${moveFallback}xdotool click --repeat ${clicks} ${btn}`;
    const xExpr = hasCoords ? '"$SX"' : '"$((W / 2))"';
    const yExpr = hasCoords ? '"$SY"' : '"$((H / 2))"';
    const args = `$(printf '{"x":%s,"y":%s,"direction":"${direction}","amount":${clicks},"by":"line","scope":"desktop","session":"${REMOTE_CUA_SESSION}"}' ${xExpr} ${yExpr})`;
    const scalePrefix = hasCoords ? `${scaled("SX", sx!, "x")}; ${scaled("SY", sy!, "y")}; ` : "";
    return `${scalePrefix}CUA_ARGS=${args}; ${cuaOrX11("scroll", '"$CUA_ARGS"', fallback)}`;
  }
  if (kind === "wait") {
...
async function call(id: unknown, name: string, args: any) {
  ...
  if (name === "drag") {
    ...
    if (!Number.isFinite(fx) || !Number.isFinite(fy) || !Number.isFinite(tx) || !Number.isFinite(ty)) {
      return text(id, "drag needs numeric from_x, from_y, to_x, to_y", true);
    }
    const what = `dragged from ${fx},${fy} to ${tx},${ty}`;
    return actAndObserve(id, [{ ...args, action: "drag" }], what, args);
  }
  if (name === "type_text") {
  ...
  if (name === "scroll") {
    ...
    const loc = Number.isFinite(Number(args.x)) && Number.isFinite(Number(args.y)) ? ` at ${Math.round(Number(args.x))},${Math.round(Number(args.y))}` : "";
    return actAndObserve(id, [{ action: "scroll", direction, clicks, x: args.x, y: args.y }], `scrolled ${direction} ${clicks}${loc}`, args);
  }
