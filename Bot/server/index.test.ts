...
beforeAll(async () => {
  ...
  writeFileSync(join(staticDir, "index.html"), "<!doctype html><title>Packaged Parallel</title>");
...
describe("harness HTTP API", () => {
  it("reconciles durable working goal cards with scheduler truth after a restart", async () => {
    ...
    expect(room.messages.find(
      ...
      text: "Goal failed: Parallel restarted before this goal finished.",
      goalRun: {
        status: "failed",
        detail: "Parallel restarted before this goal finished.",
        turnCount: 2,
  ...
  it("rejects non-loopback authorities while accepting IPv4 and IPv6 loopback forms", async () => {
    ...
    expect(Reflect.get(Object(Reflect.get(Object(brand.body), "brand")), "name")).toBe("Parallel");
  ...
  it("serves packaged UI assets and preserves API 404s", async () => {
    ...
    expect(await root.text()).toContain("Packaged Parallel");
    ...
    expect(await spa.text()).toContain("Packaged Parallel");
  ...
  it("leaves a failed credential-card continuation on the card without buzzing twice", async () => {
    ...
    try {
      ...
      expect(directCard).toMatchObject({
        kind: "secret",
        text: "Securely provide the OpenAI API key from Parallel on your phone or computer. It is never added to chat.",
      });
  ...
  it("cards every ask when Claude's reviewer never started, says so once, and can hand the allow to Claude for the session", async () => {
    ...
    // Manual, and asks about everything. Parallel passes that through —
  ...
  it("injects the bot's standing instructions (soul) into a real turn, directly after the persona", async () => {
    ...
    try {
      ...
      expect(system.startsWith("You are Kiwi, a personal bot in Parallel. Role: Tracker.")).toBe(true);
      const persona = "You are Kiwi, a personal bot in Parallel. Role: Tracker.";
  ...
  it("coaches a blank bot to set itself up, and stops once it has a description", async () => {
    ...
    try {
      ...
      expect(system.startsWith("You are Blank, a personal bot in Parallel.")).toBe(true);
  ...
  it("never removes an unmanaged container that squats on a bot's exact Local VM name", async () => {
    ...
    try {
      ...
      expect(removed.body.error).toMatch(/not created by Parallel.*remove it manually/i);
  ...
  it("previews the system prompt the model will see, section by section", async () => {
    ...
    try {
      ...
      expect(before.body.sections[0]).toEqual({
        id: "persona",
        label: "Identity",
        text: "You are Kiwi, a personal bot in Parallel. Role: Tracker. About: Files bugs.",
        bytes: 78,
      });
