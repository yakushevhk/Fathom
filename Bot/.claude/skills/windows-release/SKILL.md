...
description: Build and verify the Windows desktop build (NSIS installer + latest.yml) for the canonical Parallel release and its legacy updater mirror. Use when cutting a release, shipping a new version to Windows users, or when a Windows user reports they are stuck on an old version. Windows only — does not cover the macOS dmg/notarization flow.
---

# Windows release

Ships `Parallel-<version>-setup.exe` and its update feed to
[milind-soni/Parallel](https://github.com/milind-soni/Parallel/releases).
...
| File | Purpose |
|---|---|
| `Parallel-<version>-setup.exe` | the installer |
| `latest.yml` | **the update feed** — see step 4 |
| `Parallel-<version>-setup.exe.blockmap` | differential updates |
| `Parallel-<version>-x64.zip` | portable, not used by the updater |
...
- `app-update.yml` must point at `milind-soni/Parallel` and, while the
...
2. The chat window renders (not the error page). Server logs land in
   `%APPDATA%\Parallel\logs\server.log`.
...
```powershell
Copy-Item release/Parallel-<version>-setup.exe release/Parallel-setup.exe
gh release upload v<version> --repo milind-soni/Parallel `
  release/Parallel-<version>-setup.exe `
  release/Parallel-setup.exe `
  release/Parallel-<version>-setup.exe.blockmap `
  release/latest.yml
```
...
- **`Parallel-<version>-setup.exe`** is what `latest.yml` references by name and
  sha512. The auto-updater downloads exactly this.
- **`Parallel-setup.exe`** is a byte-identical copy that gives the README's
  `/releases/latest/download/Parallel-setup.exe` button a stable URL. This
  mirrors `Parallel.dmg` sitting beside `Parallel-<version>.dmg`.
